# Functions for adding ClassyFire classifications
...
my_column <- Standards %>%
  select(InChI.Key.Name) %>%
  as.tibble() %>%
  slice(1:5)

# parse_json_output -------------------------------------------------------
parse_json_output <- function(json_res) {
  list_output <- list(kingdom = json_res[['kingdom']],
  ...
  if (length(list_output) > 0) {
    class_tibble <- purrr::map(1:length(list_output), ~{
      l <- list_output[[.]]
      tibble::tibble(Level = names(list_output)[.],
...
get_classification_2 <- function(inchi_key) {
  ...
  if (response$status_code == 200) {
    ...
    labeled_classification <- classification %>%
      mutate(InChI.Key.Name = inchi_key) %>%
      select(InChI.Key.Name, everything())
...
classyfied_test <- high_class(my_column)

classyfied_test_concat <- classyfied_test %>%
  unite(Combined, c("Level", "Classification"), sep = ": ") %>%
  arrange(InChI.Key.Name) %>%
  group_by(InChI.Key.Name) %>%
  mutate("Classyfire" = paste(Combined, collapse = "; ")) %>%
  select(-Combined, -CHEMONT) %>%
  unique() %>%
  as.data.frame()

full_test_frame <- classyfied_test_concat %>%
  left_join(Standards, by = "InChI.Key.Name")
